...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [menuItemsRes, ingredientsRes, prepRecipesRes] = await Promise.all([
      ...
      supabase
        .from("ingredients")
        .select("id, name, unit, unit_cost_cents")
        .eq("restaurant_id", restaurant_id),
      supabase
        .from("prep_recipes")
...
- kind="ingredient": ingredientId MUST be one of the real ids in AVAILABLE INGREDIENTS below (never invent one); unit MUST exactly match that ingredient's own unit as shown; quantity is your best real per-serving usage estimate. prepRecipeId, proposedName, proposedSubIngredients null.
- kind="prep_recipe": same, but ingredientId null, prepRecipeId one of AVAILABLE PREP RECIPES' real ids, unit MUST exactly match that prep recipe's own yield unit.
...
${JSON.stringify(ingredients.map((i: { id: string; name: string; unit: string; unit_cost_cents: number | null }) => ({ id: i.id, name: i.name, unit: i.unit })))}
