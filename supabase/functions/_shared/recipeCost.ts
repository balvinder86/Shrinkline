// Ported from src/lib/boh/recipeCost.ts for the chat edge function's
// P&L/Cost Variance/Inventory Variance tools — pure, dependency-free
// logic (only import is units.ts), but Deno edge functions in this
// repo don't share a build step with the Vite app, so it's duplicated
// rather than imported across runtimes. Keep in sync by hand if the
// source file changes — this exists so the chat assistant's numbers
// match the dashboard pages exactly, not a second, divergent
...
import { convertQuantityToIngredientUnit } from "./units.ts";
...
// Base cost — an item's own single, untiered recipe. The chat tools
// intentionally do not port resolveBlendedTierCostCents (price-tier
// blending) — a v1 simplification documented in the chat plan; a
// tiered item's true blended cost can differ slightly from what this
// resolves.
...
// dollar contribution instead of collapsing to one total — used by
// get_food_cost_variance to show which specific ingredients explain
...
// contract as the two functions above.
...
function accumulatePrepRecipeIngredientUsage(
  ...
  // infinite recursion here would hang the whole request rather than
  // just mis-costing one item, so it's worth the defensive check.
...
// be set. Used by get_inventory_variance, which reconciles physical
// counts against purchases/usage/waste in real units; most ingredients
// don't have a cost yet, and a quantity reconciliation has no reason
// to go blind on them just because pricing hasn't caught up. Still
// returns false — meaning "discard whatever this call added to `acc`"
// — when a line's unit can't convert to the ingredient's own unit (no
// container-size bridge), since that's a real "we don't know how much
// of this was used" gap, not merely a costing gap.
