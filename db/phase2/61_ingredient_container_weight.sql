-- Extends 60_ingredient_container_size.sql's volume bridge with an
-- equivalent weight bridge — a case-purchased food item (chicken,
-- flour) needs "how many grams is one case" the same way a
-- bottle-purchased liquor needed "how many ml is one bottle."
alter table ingredients add column if not exists container_size_g numeric;
alter table ingredients drop constraint if exists ingredients_container_size_g_check;
alter table ingredients add constraint ingredients_container_size_g_check
  check (container_size_g is null or container_size_g > 0);

-- Mirrors src/lib/units.ts's convertQuantityToIngredientUnit exactly
-- (same mL-per-unit / g-per-unit ratios, now including kg and weight
-- oz alongside the original volume units) — keep both in sync if the
-- unit list ever changes.
create or replace function convert_to_ingredient_unit(
  p_quantity numeric,
  p_from_unit text,
  p_ingredient_unit text,
  p_container_size_ml numeric,
  p_container_size_g numeric default null
) returns numeric
language sql
immutable
as $$
  select case
    ...
    when p_from_unit in ('lb','oz_wt','kg','g') and p_ingredient_unit in ('lb','oz_wt','kg','g') then
      p_quantity
      * (case p_from_unit when 'lb' then 453.592 when 'oz_wt' then 28.3495 when 'kg' then 1000 when 'g' then 1 end)
      / (case p_ingredient_unit when 'lb' then 453.592 when 'oz_wt' then 28.3495 when 'kg' then 1000 when 'g' then 1 end)
    when p_from_unit in ('oz','cup','pt','qt','gal','ml','L')
     and p_ingredient_unit not in ('oz','cup','pt','qt','gal','ml','L','lb','oz_wt','kg','g')
      ...
      * (case p_from_unit
      ...
      / p_container_size_ml
    when p_from_unit in ('lb','oz_wt','kg','g')
     and p_ingredient_unit not in ('oz','cup','pt','qt','gal','ml','L','lb','oz_wt','kg','g')
     and p_container_size_g is not null then
      p_quantity
      * (case p_from_unit when 'lb' then 453.592 when 'oz_wt' then 28.3495 when 'kg' then 1000 when 'g' then 1 end)
      / p_container_size_g
    else null
...
-- Same shape as 60's version, plus the weight-container join/param so
-- a case-purchased food ingredient's usage sum converts correctly too.
...
begin
  ...
  from (
    select
      ...
      sum(
        coalesce(
          convert_to_ingredient_unit(
            rl.quantity, rl.unit, ing.unit, ing.container_size_ml, ing.container_size_g
          ),
          rl.quantity
        ) * ps.quantity_sold
